...
| 탭 | 내용 |
|----|------|
| **개요** | 인스턴스/DB 정보 + 성능 지표 14종 + 지표별 최근 1시간 sparkline + 세션 요약 + **✨ 지금 상황 AI 요약**(현재 스냅샷 3줄 진단) |
| **대시보드** | Grafana 스타일 실시간 차트(상태 타일 6 + 시계열 6) + **기준선 비교(어제 vs 오늘)** + 최근 CPU 스파이크 이력 |
| **ASH** | **자체 액티브 세션 히스토리** — 구간별 Top SQL/대기이벤트/세션 + AAS 추이 + **액티비티 히트맵(대기클래스 × 시간, OEM 스타일)** (Diagnostic Pack 라이선스 불필요) |
...
- **조회 로딩 표시** — 탭 진입/새로고침이 140ms 넘게 걸리면 **"⏳ 조회 중입니다…"** 오버레이 표시(빠른 탭은 깜빡임 없이 생략)
- **행 확장 상세**(넓은 표 클릭 시 key/value 펼침) + **표 밀도(compact) 토글**(▤)
- 마지막 탭·주기 기억(localStorage)
- **다크/라이트 테마 토글**(🌙/☀️ 버튼, localStorage 유지, 새로고침 시 깜빡임 없음)
...
**지금 상황 AI 요약** — 개요 탭의 **✨ 요약 생성** 버튼은 현재 스냅샷(세션 요약·Top SQL·활성 대기·블로킹)을 Claude(effort `low`)에 보내 **상태 한 줄 + 핵심 관찰 + 주의점**을 한국어로 받습니다. 비용 절감을 위해 **120초 메모리 캐시**로 동작하며, 튜닝 제안과 동일하게 API 키가 있을 때만 활성화됩니다.
